...
AnalogIn encoder_1(8, A1, BIT8);
AnalogIn encoder_2(10, 9, BIT8);
AnalogIn encoder_3(6, 12, BIT8);
AnalogIn encoder_4(4, MOSI, BIT8);
AnalogIn encoder_5(0, 1, BIT8);
AnalogIn encoder_sw(A2, 11, BIT8);


AnalogIn encoder_6(5, 13, BIT8);
AnalogIn encoder_7(A5, A3, BIT8);
...
Joystick_ Joystick[JOYSTICK_COUNT] = {
  Joystick_(0x03, JOYSTICK_TYPE_JOYSTICK,  32, 2,  true, true, false, false, false, false, false, false, false, false, false),
...
unsigned short prvenc1 = 0;

unsigned short enc2 = 0;
unsigned short prvenc2 = 0;

unsigned short enc3 = 0;
unsigned short prvenc3 = 0;

unsigned short enc4 = 0;
unsigned short prvenc4 = 0;

unsigned short enc5 = 0;
unsigned short prvenc5 = 0;

unsigned short enc6 = 0;
unsigned short prvenc6 = 0;

unsigned short enc7 = 0;
unsigned short prvenc7 = 0;

unsigned short encsw = 0;
unsigned short prvencsw = 0;
...
#define WS2812B_DATAPIN 7            //{"Name":"WS2812B_DATAPIN","Title":"Data (DIN) digital pin number","DefaultValue":"6","Type":"pin;WS2812B LEDS DATA","Condition":"WS2812B_RGBLEDCOUNT>0"}
...
volatile int enc_release_cnt = 0;
...
uint16_t butdat_ex = 0;     // Button data
uint16_t prvdat_ex = 0;     // Previous button data
uint16_t button_ex = 0;     // Button data store
uint16_t prvbut_ex = 0;     // Previous button data
uint16_t presdb_ex = 0;     // Pressed button
unsigned char vdcnt_ex = 0;      // Valid data counter
unsigned char lvdcnt_ex = 0;     // Long valid data counter

uint16_t butdat_1 = 0;     // Button data
uint16_t prvdat_1 = 0;     // Previous button data
uint16_t button_1 = 0;     // Button data store
uint16_t prvbut_1 = 0;     // Previous button data
uint16_t presdb_1 = 0;     // Pressed button
unsigned char vdcnt_1 = 0;      // Valid data counter
unsigned char lvdcnt_1 = 0;     // Long valid data counter

uint16_t butdat_2 = 0;     // Button data
uint16_t prvdat_2 = 0;     // Previous button data
uint16_t button_2 = 0;     // Button data store
uint16_t prvbut_2 = 0;     // Previous button data
uint16_t presdb_2 = 0;     // Pressed button
unsigned char vdcnt_2 = 0;      // Valid data counter
unsigned char lvdcnt_2 = 0;     // Long valid data counter



void encoder_buttons_release(void)
{
  for(int i = 1; i<17; i++)
  {
    Joystick[1].releaseButton(i);  
  }
}
...
ISR (TIMER1_COMPA_vect) {
  ...
  if(enc_release_cnt > 0)
  {
    --enc_release_cnt;
    if(enc_release_cnt == 0)
    {
      encoder_buttons_release();
    }
  }
...
void epts_setup()
{
  ...
  setTimer1(0.01);
  pin_init();
  encoder_1.setSpeed(1);
  encoder_2.setSpeed(1);
  encoder_3.setSpeed(1);
  encoder_4.setSpeed(1);
  encoder_5.setSpeed(1);
  encoder_6.setSpeed(1);
  encoder_7.setSpeed(1);
  encoder_sw.setSpeed(1);
        
  Serial.begin(11520);
  delay(200);

  gpio1.begin(PCAL9535A::HardwareAddress::A000);      // use default address 0x20
  gpio2.begin(PCAL9535A::HardwareAddress::A001);      // use address 0x21

  for(int i= 0; i<16; i++)
  {
    gpio1.pinMode(i, INPUT);
    delay(20);
    gpio2.pinMode(i, INPUT);
    delay(20);   
    gpio1.pinSetPull(i, PCAL9535A::PullSetting::UP);  // turn on the internal pullup
    delay(20);
    gpio2.pinSetPull(i, PCAL9535A::PullSetting::UP);  // turn on the internal pullup
    delay(20);    
  }
...
void butread_gpio_1(void)
{

  butdat_1 = gpio1.readGPIO16();

  // BUT_EX_1 NO to NC
  if(butdat_1 & 0x0002)
  {
    butdat_1 = butdat_1 & 0xFFFD;
  }
  else
  {
    butdat_1 = butdat_1 | 0x0002;
  }
     
  if (butdat_1==prvdat_1)
  {
    vdcnt_1++;
    if (button_1 != butdat_1)     //Long button control
    {
      if (vdcnt_1 == 5)     //5 * 10ms = 50ms debounce
      {
        vdcnt_1 = 0;
        prvbut_1 = button_1;
        button_1 = butdat_1;
        presdb_1 = (butdat_1 ^ prvbut_1) & prvbut_1;
        if (presdb_1 != 0)
        {
          if (presdb_1 & 0x8000)       
          {
          ...
          if (presdb_1 & 0x2000)     
          {
             Joystick[0].pressButton(3);  
          }
          if (presdb_1 & 0x1000)          
          {
             Joystick[0].pressButton(4); 
          }
          if (presdb_1 & 0x0800)           
          {
             Joystick[0].pressButton(5);  
          }
          if (presdb_1 & 0x0400)     
          {
             Joystick[0].pressButton(6);  
          }
          if (presdb_1 & 0x0200)  
          {
             Joystick[0].pressButton(7); 
          }          
          if (presdb_1 & 0x0100)  
          {
             Joystick[0].pressButton(8);   
          }
          if (presdb_1 & 0x0040)     
          {
             Joystick[0].pressButton(10);  
             // 7-way sağ sol yukarı ve aşağı basımlarda push da basıyor, bunu iptal ettik.
             f_sw_side_push = true;
          }
          if (presdb_1 & 0x0020)     
          {
             Joystick[0].pressButton(11); 
             // 7-way sağ sol yukarı ve aşağı basımlarda push da basıyor, bunu iptal ettik.
             f_sw_side_push = true;  
          }
          if (presdb_1 & 0x0010)          
          {
             Joystick[0].pressButton(12);  
             // 7-way sağ sol yukarı ve aşağı basımlarda push da basıyor, bunu iptal ettik.
             f_sw_side_push = true;
          }
          if (presdb_1 & 0x00008)           
          {
             Joystick[0].pressButton(13);  
          }
          if (presdb_1 & 0x0004)     
          {
             Joystick[0].pressButton(14);  
             // 7-way sağ sol yukarı ve aşağı basımlarda push da basıyor, bunu iptal ettik.
             f_sw_side_push = true;            
          }
          if (presdb_1 & 0x0002)  
          {
             Joystick[0].pressButton(15); 
          }          
          if (presdb_1 & 0x0001)  
          {
             Joystick[0].pressButton(16);                     
          }

          //
          if (presdb_1 & 0x0080)    // !!!!!!!!!!!!!!!!!!!!!!! YERI DEĞİŞTİ,   7-WAY TUŞ BASIMINI DÜZELTMEK İÇİN !!!!!!!!!!!!!!!!!!!  
          {
             if(!f_sw_side_push)
             {
                Joystick[0].pressButton(9);  
             }
          }
          //
        }
...
void butread_ex_button(void)
{

  butdat_ex = digitalRead(MISO);    
  if (butdat_ex==prvdat_ex)
  {
    vdcnt_ex++;
    if (button_ex != butdat_ex)     //Long button control
    {
      if (vdcnt_ex == 5)     //5 * 10ms = 50ms debounce
      {
        vdcnt_ex = 0;
        prvbut_ex = button_ex;
        button_ex = butdat_ex;
        presdb_ex = (butdat_ex ^ prvbut_ex) & prvbut_ex;
        if (presdb_ex != 0)
        {
          if (presdb_ex & 0x0001)       
          {
            Joystick[1].pressButton(17);       
          }
        }
        else
        {
           Joystick[1].releaseButton(17);  
        }
      }
    }
  }
...
void butread_gpio_2(void)
{

  butdat_2 = gpio2.readGPIO16();
  // BUT_EX_3 NO to NC
  if(butdat_2 & 0x8000)
  {
    butdat_2 = butdat_2 & 0x7FFF;
  }
  else
  {
    butdat_2 = butdat_2 | 0x8000;
  }
    
  if (butdat_2==prvdat_2)
  {
    vdcnt_2++;
    if (button_2 != butdat_2)     //Long button control
    {
      if (vdcnt_2 == 5)     //5 * 10ms = 30ms debounce
      {
        vdcnt_2 = 0;
        prvbut_2 = button_2;
        button_2 = butdat_2;
        presdb_2 = (butdat_2 ^ prvbut_2) & prvbut_2;
        if (presdb_2 != 0)
        {
          if (presdb_2 & 0x8000)       
          {
            Joystick[0].pressButton(17);        
          }
          if (presdb_2 & 0x4000)     
          {
            Joystick[0].pressButton(18);   
          }
          if (presdb_2 & 0x2000)     
          {
            Joystick[0].pressButton(19);      
          }
          if (presdb_2 & 0x1000)          
          {
            Joystick[0].pressButton(20);   
          }
          if (presdb_2 & 0x0800)           
          {
            Joystick[0].pressButton(21);    
          }
          if (presdb_2 & 0x0400)     
          {
            Joystick[0].pressButton(22);    
          }
          if (presdb_2 & 0x0200)  
          {
            Joystick[0].pressButton(23);   
          }          
          if (presdb_2 & 0x0100)  
          {
            Joystick[0].pressButton(24);      
          }
          if (presdb_2 & 0x0080)       
          {
            Joystick[0].pressButton(25);          
          }
          if (presdb_2 & 0x0040)     
          {
            Joystick[0].pressButton(26);   
          }
          if (presdb_2 & 0x0020)     
          {
            Joystick[0].pressButton(27);    
          }
          if (presdb_2 & 0x0010)          
          {
            Joystick[0].pressButton(28);    
          }
          if (presdb_2 & 0x00008)           
          {
            Joystick[0].pressButton(29);    
          }
          if (presdb_2 & 0x0004)     
          {
            Joystick[0].pressButton(30);    
          }
          if (presdb_2 & 0x0002)  
          {
            Joystick[0].pressButton(31);   
          }          
          if (presdb_2 & 0x0001)  
          {
            Joystick[0].pressButton(32);                       
          }
        }
        else
        {
...
{
    enc2 = encoder_2.read(enc2);
    if(enc2 != prvenc2)
    {
      enc_release_cnt = 5;
      if(enc2 > prvenc2)
      {
        Joystick[1].pressButton(3); 
      }
      else
      {
        Joystick[1].pressButton(4); 
      }
      prvenc2 = enc2;
    }
}

void read_encoder_3(void) 
{
    enc3 = encoder_3.read(enc3);
    if(enc3 != prvenc3)
    {
      enc_release_cnt = 5;
      if(enc3 > prvenc3)
      {
        Joystick[1].pressButton(5); 
      }
      else
      {
        Joystick[1].pressButton(6); 
      }
      prvenc3 = enc3;
    }
}
...
void read_encoder_4(void) 
{
    enc4 = encoder_4.read(enc4);
    if(enc4 != prvenc4)
    {
      enc_release_cnt = 5;
      if(enc4 > prvenc4)
      {
        Joystick[1].pressButton(7); 
      }
      else
      {
        Joystick[1].pressButton(8); 
      }
      prvenc4 = enc4;
    }
}
...
void read_encoder_5(void) 
{
    enc5 = encoder_5.read(enc5);
    if(enc5 != prvenc5)
    {
      enc_release_cnt = 5;
      if(enc5 > prvenc5)
      {
        Joystick[1].pressButton(9); 
      }
      else
      {
        Joystick[1].pressButton(10); 
      }
      prvenc5 = enc5;
    }
}
...
void read_encoder_6(void) 
{
    enc6 = encoder_6.read(enc6);
    if(enc6 != prvenc6)
    {
      enc_release_cnt = 5;
      if(enc6 > prvenc6)
      {
        Joystick[1].pressButton(11); 
      }
      else
      {
        Joystick[1].pressButton(12); 
      }
      prvenc6 = enc6;
    }
}

void read_encoder_7(void) 
{
    enc7 = encoder_7.read(enc7);
    ...
    if(enc7 != prvenc7)
    {
      ...
      if(enc7 > prvenc7)
      {
        Joystick[1].pressButton(13); 
      }
      else
      {
        Joystick[1].pressButton(14); 
      }
      prvenc7 = enc7;
    }
}
...
void read_encoder_sw(void) 
{
    encsw = encoder_sw.read(encsw);
    Serial.println(encsw);
    if(encsw != prvencsw)
    {
      enc_release_cnt = 5;
      if(encsw > prvencsw)
      {
        Joystick[1].pressButton(15); 
      }
      else
      {
        Joystick[1].pressButton(16); 
      }
      prvencsw = encsw;
    }
}
...
{
   int8_t lx, ly, rx, ry;
   Val1=analogRead(A4);            //We read both values and display them raw on the serial monitor
